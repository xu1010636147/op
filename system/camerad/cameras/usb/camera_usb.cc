...
static int mjpeg_to_nv12(const uint8_t *jpeg_data, size_t jpeg_size, uint8_t *nv12_data, int width, int height) {
  ...
  // NV12: Y plane (height * width) + UV interleaved (height/2 * width)
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    for (int x = 0; x < width; x++) {
      // Y分量写入Y平面
      nv12_y[row * width + x] = row_buffer[x * cinfo.output_components];
      // UV分量隔行交织写入NV12的UV平面
      if (row % 2 == 0 && x % 2 == 0) {
        int uv_idx = (row / 2) * (width / 2) + (x / 2);
        nv12_uv[uv_idx * 2] = row_buffer[x * cinfo.output_components + 1];     // U
        nv12_uv[uv_idx * 2 + 1] = row_buffer[x * cinfo.output_components + 2]; // V
      }
    }
    row++;
  }
