...
CarrotPanel::CarrotPanel(QWidget* parent) : QWidget(parent) {
  ...
  startToggles->addItem(new CValueControl("EnableRadarTracks", "启用雷达追踪(1)", "1:启用雷达追踪, -1,2:禁用 (始终使用HKG SCC雷达)，胜达设置为1, 改变值后需要重启车辆", -1, 2, 1));
  ...
  navToggles->addItem(new CValueControl("ContinuousLaneChangeInterval", "ATC 连续变道时间间隔(5秒)", "变道后允许再次变道的时间间隔(秒)", 0, 30, 1));
